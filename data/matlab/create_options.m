function mpopt = create_options(mpc, solver, start, voltage, balance, model)
    %create_options Creates matpower options structure.
    %
    % mpopt = create_options(mpc, solver, start, voltage, balance)
    %
    % Outputs
    ...
    %   model   - AC or DC OPF model
        ...
        elseif (solver == MIPS)
            ...
            mpopt = mpoption(mpopt, 'opf.dc.solver', 'MIPS', 'mips.step_control', 0);
